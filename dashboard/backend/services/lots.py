...
It deliberately does NOT re-pair closed cycles (that is FC-020 proper), and
its output must never be summed into P&L: under the dashboard's accounting
convention, share acquisition cost is already expensed in the OPTRD cash
ledger (`share_side_pnl`), so MTM P&L uses full market value of held
shares, not (price − basis) × shares. See docs/plans/fc-031.md.
...
def open_lots(optrd_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    is skipped; callers can detect it via the ``unpaired_sells`` count.

    Returns lots as ``{"qty", "price", "buy_time"}`` oldest-first.
    """
    ...
    for ev in sorted(optrd_events, key=lambda e: str(e.get("transaction_time") or "")):
        ...
        if remaining > 0:
            unpaired += 1
    for lot in lots:
        lot["unpaired_sells"] = unpaired  # surfaced on every row for callers
    return lots
...
def open_lot_basis(optrd_events: List[Dict[str, Any]]) -> Dict[str, Optional[float]]:
    ...
    """
    lots = open_lots(optrd_events)
    shares = sum(lot["qty"] for lot in lots)
    if shares <= 0:
        unpaired = lots[0]["unpaired_sells"] if lots else 0
    ...
    return {
        ...
        "unpaired_sells": lots[0]["unpaired_sells"],
    }
