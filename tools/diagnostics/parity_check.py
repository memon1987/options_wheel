...
Usage:
    python tools/diagnostics/parity_check.py                    # whole history
...
def fetch_live_put_sales(config):
    """Every put sell-to-open fill on the account, oldest first."""
    ...
    for a in rows:
        ...
        if not m or m.group(3) != "P" or a.get("side") != "sell_short":
            continue
...
def main() -> int:
    ...
    config = Config()
    max_dte = args.max_dte if args.max_dte is not None else getattr(config, "put_target_dte", 7)

    sales = fetch_live_put_sales(config)
    if args.symbols:
    ...
    print(f"\nParity check: {len(decisions)} live decisions, max_dte={max_dte}\n")
    ...
    for d in decisions:
        ...
        best = suitable[0]  # exactly what PutSeller.find_put_opportunity takes
...
def _summarize(results):
    ...
    if matched:
        ...
        if deltas:
            in_band = sum(1 for x in deltas if 0.10 <= x <= 0.20)
            print(f"  sim delta in [0.10,0.20]: {in_band}/{len(deltas)} "
